...
/**
 * Official brand marks (Simple Icons, single-path, 24×24 viewBox). `fill`
 * defaults to `currentColor` so the mark inherits the surrounding text color —
 * inside a `SegmentedOption` it greys when idle and tints primary when the
 * option is selected, matching the control's language. Decorative by default
 * (`aria-hidden`); the accompanying label carries the accessible name.
 */
export function ChessComLogo(props: SVGProps<SVGSVGElement>) {
    return (
...
export function LichessLogo(props: SVGProps<SVGSVGElement>) {
    return (
        <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden {...props}>
...
export function FaceitLogo(props: SVGProps<SVGSVGElement>) {
    return (
        <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden {...props}>
