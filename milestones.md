...
- [ ] **Remaining (ongoing):** the listing **row** + **profile** cards got the W/L removal but NOT a spacing-rhythm pass — revisit. Verify responsive at 375 / 768 / 1024. Gates this session: host `tsc` / `eslint` / `prettier` / `vite build` green throughout; full backend suite **1665** (last full run before the FE-only polish). User will "continue fixing some parts" of the cards after a /compact.
