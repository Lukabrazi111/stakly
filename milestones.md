...
- **M41 — Verified skill ratings (display)** — replace the free-typed create-listing skill range with each player's **real, API-pulled rating** (FACEIT ELO + derived level; chess.com / Lichess per-time-control ratings), shown on listings + profiles. **Display-only — never gates a match** (taker's choice); the self-typed skill inputs come off the create form. FACEIT first, chess second. **Decided 2026-06-26:** chess listings become **single time-control** (one platform + TC → one unambiguous rating); the asymmetric "punch-up-only" gate is **deferred** (ship display first; revisit only as a non-blocking mismatch *warning*, never a block). **P1–P7 + P6 all done (2026-06-28); P8 (card redesign) largely done.** **Closed 2026-06-30.** The deferred FACEIT-unification sub-item is resolved as a **non-goal** (keep the scalar — see P6); the P8 responsive pass is done (code audit + a real Playwright browser sweep at 375 / 768 / 1024). The browser sweep caught + fixed a genuine bug — the 3-col lobby grid overflowed ~116px at the `lg` boundary; **gated to `xl`**. It also surfaced a **separate, pre-existing** site-header overflow at ~768 for authed users (NOT M41 — flagged for a follow-up). Full spec + phases in the section below.
...
- **FACEIT level → circular dial in authentic FACEIT colors** (official ladder: grey L1, green L2–3, yellow L4–7, orange L8–9, red L10). **Reverses P2's "never FACEIT amber/red near a stake."** Contained by keeping the dial unmistakably a *level* (centered number + ring form + its own placement) so green-7 / orange-10 never reads as win/dispute status next to the stake.
...
- [x] **FACEIT level dial** replaces the flat `FaceitRatingBadge` pill — an SVG ring filled proportionally to level (level-9 ≈ 90%), number centered in `text-foreground` (WCAG AA), ELO beside it. **Authentic FACEIT colors** (5 CSS vars in `app.css` + `faceit-level.ts` helper; **reverses P2's no-amber call**) — official ladder: grey L1, green L2–3, yellow L4–7, orange L8–9, red L10. Compact + detail variants. Lands on every `FaceitRatingBadge` surface (grid cards, rows, profile rows, create preview, detail).
- [x] **Recent-form strip (W / L / D chips)** — last 5 **settled CS2 matches** for the player. **Win/loss read from the LEDGER, not `winner_user_id`** (team matches stamp only the slot-0 winner, so a non-slot-0 winner would mis-read as a loss): a player won iff they got a `Wallet::payout` for that match; draw = `winner_user_id` null on a settled match; else loss. New `App\Services\RecentForm` (3 queries, batch, no N+1 — mirrors `SellerTrust::attachTo`); `ListingResource.creator.recent_form` (CS2 only); `RecentFormStrip` (status palette green/red/grey). **Stakly DB only — no FACEIT history API.** Game-scoped to CS2 (coherent with the dial; widen later if wanted). **Lobby-only (2026-06-28):** the strip was dropped from the marketplace cards (grid / row / profile) — they show the completion-rate trust meta instead; the W/L strip lives only on the lobby `SlotCard` (vertical, full-height, always 5 slots, padded with a faded "N" for no-match-yet).
...
- [x] **P7c — lobby slot cards (`SlotCard`).** `LobbyResource::presentParticipant` now carries `faceit_rating` (level derived from the platform ELO) + `recent_form` (batch-loaded via `RecentForm::forBatch($userIds, $listing->game)` in `showTeamPlay`, alongside the existing `seller_trust` / `platform_stats` batches). `SlotCard` swaps the plain rating box for the `FaceitRatingBadge` dial + renders `RecentFormStrip` (vertical, flush right-edge column). Test: `LobbyRosterRatingTest` (rated→dial level + ['W'] form via ledger; unrated→level null). 144 lobby tests + new test green; host build/tsc/eslint green. **Dev-seed (2026-06-28):** `MatchHistorySeeder::seedCs2TeamHistory` gives every FACEIT user **5** settled CS2 2v2 matches via the real `SettleTeamMatchAction` flow (ledger-consistent — 0 invariant violations) so the W/L strip shows 5 coloured chips locally. Without seeded history the strip shows faded "N" slots (no match yet).
...
- [x] **Resolved 2026-06-30 — NOT doing it (non-goal).** Decided (with the user) to **keep FACEIT's scalar `linked_accounts.skill_rating`**. A single-rating provider (FACEIT, and future Steam / Riot MMR) is honestly modelled by the scalar; forcing it into the per-TC `linked_account_ratings` table would need a nullable / sentinel `time_control` (muddies the one-row-per-TC invariant + its unique key) and churn settlement-adjacent code (`TakeListingAction::snapshotRatingFor`) + the CS2 filter `EXISTS` query — all for zero user-facing gain. The two rating shapes — **scalar for single-rating providers, per-TC table for chess** — are the correct domain model, not a wart to unify.
...
Started as spacing polish; grew into the full FACEIT-roster card treatment + a run of iterative fixes driven by the user's reference screenshots. The reference's Avg HS / Avg K/D stay out of scope (need the FACEIT history API, ruled out) — the stat block uses our DB stats. `ui-ux-pro-max` active; inside the Stakly token system.

- [x] **Lobby `SlotCard` → FACEIT-roster layout.** Header (avatar + name left | ELO + level dial + Ready right), divider, an **"OVERALL" stat block** (Matches / Win rate / Completion-30d), and a **flush full-height vertical W/L column** on the right edge — always **5 slots**, padded with a faded **"N"** for no-match-yet. Full height via `self-stretch` (NOT `h-full`, which collapsed it to content size). `EmptySlot` height bumped to `128px` to track. Dial is `dialSize={30}` here.
- [x] **FACEIT level dial** (`faceit-rating-badge.tsx`) reworked to the reference look: bare **`ELO (dial)`** — number first, dial right, **no pill, no "ELO" label**. Configurable **`dialSize`** prop (marketplace default **25**, lobby **30**) + tunable module consts **`RING_THICKNESS` (0.11)** / **`LEVEL_NUMBER_SIZE` (0.4)**. Colors corrected to the **official FACEIT ladder** (grey L1 / green L2–3 / yellow L4–7 / orange L8–9 / red L10) — 5 `--faceit-*` CSS vars in `app.css`.
- [x] **Listing grid card** (home featured + `/listings`): CS2 rating moved to the **owner row** (right of name; **click-through → lobby** — only nickname/avatar → profile); the **trust meta ("100% · N matches") moved to its own full-width row** below the name (fixes the cramped-column "N matches" wrap on narrow/home cards). **W/L strip removed from ALL marketplace cards** (grid / row / profile) — it's now **lobby-only**; the completion-rate trust meta is the marketplace signal.
- [x] **Supporting:** `RecentFormStrip` gained a vertical variant + `slots`/"N" padding; `SellerTrustMeta` makes "· N matches" a nowrap unit (defensive). **Dev seed** `MatchHistorySeeder::seedCs2TeamHistory` = 5 settled CS2 2v2s per FACEIT user (real `SettleTeamMatchAction`, ledger-consistent — 0 invariant violations) so the lobby W/L shows 5 chips locally.
- [x] **Chess grid card → CS2 grid styling (2026-06-28).** The chess card's verified rating moved from a tier-tinted pill in the match-meta row UP to the **owner row** (right of the name, click-through to the card), matching the CS2 card. New `bare` variant on `ChessRatingBadge` renders just the high-contrast Elo number (+ "?"), mirroring `FaceitRatingBadge`'s compact ELO — **no FACEIT dial** (chess has no level). Match-meta row now carries only time-control + languages.
- [x] **Row + profile cards → `bare` chess rating (2026-06-28).** Chess rating switched to the `bare` variant in `listing-row.tsx` + `profile-listing-row.tsx` (consistency with CS2, already bare inline in those chip strips). Spacing reviewed post-W/L-removal — clean, no leftover gaps, no change needed. The tinted pill now only remains on the create preview + listing-detail surfaces. Gates: host `tsc` / `eslint` / `prettier` / `vite build` green.
- [x] **Create-form preview card re-synced to the grid card (2026-06-28).** `ListingPreviewCard` (the "twin" of `ListingGridCard`) restructured to match the P8 layout: verified rating moved to the **owner row** (right of name — CS2 → ELO + dial, chess → bare ELO), region + verified trust on its **own row** below, and the rating removed from the chip strip (now time-control + languages + private only). The create preview now faithfully shows how the posted card will look.
- [x] **Responsive verification** at 375 / 768 / 1024 — **done 2026-06-30 via a real Playwright browser sweep** (the earlier code audit missed the bug below). All M41 rating surfaces verified clean across the three widths: marketplace **grid** (chess bare-ELO + CS2 FACEIT dial on the owner row, trust meta on its own row), **rows**, **profile rows**, **listing detail** ("Verified rating" chip), **create preview**, and the **lobby slot cards** (ELO + authentic-color level dial, 3-col OVERALL block, flush full-height W/L column). Horizontal-overflow checked programmatically (`scrollWidth` vs `clientWidth`) at every surface, ignoring the intentionally-`w-max` marquee. **Bug found + fixed:** the 3-col lobby grid (`team-play-lobby-view.tsx`) was gated at `lg` (1024), but the dense P8 slot card's min-content (~300px) + the `minmax(360px,400px)` center column overflowed the two `1fr` tracks (~116px h-scroll; Team B's column spilled off-screen). **Gated the 3-col layout to `xl` (1280)** — below that it uses the clean stacked layout (the same one that renders perfectly at 375). Verified `scrollWidth == clientWidth` at 1024 after the fix. (My earlier audit wrongly assumed the roster stacked below `lg`; it didn't — the browser pass is what caught it.)

> **Separate finding (NOT a rating surface; pre-existing) — site-header overflow at ~768, FIXED 2026-06-30.** For *authed* users the `SiteHeader` overflowed ~37px across the `md`–`lg` range: the desktop nav turns on at `md` (768) while the wallet-balance pill + (for unverified users) the verify-nudge chip stayed inline, exceeding the width (guests were fine — narrower header). **Fix:** the two secondary authed-only chips now hide below `lg` — `BalanceChip` bumped `md:inline-flex` → `lg:inline-flex`, `UnverifiedChip` gets `max-lg:hidden`. Balance stays reachable via the profile-menu Wallet link (md–lg) + the `MobileMenu` (<md); the verify nudge still surfaces via its banner + the disabled Create-listing tooltip. Verified in-browser via Playwright: no overflow at 768, balance reappears at 1024 (`lg`).
