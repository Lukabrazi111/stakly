...
- **M34 Phase 3.1** — Unify lobby into the listing page. Frontend follow-up to P3: collapse the dedicated `/lobbies/{id}` URL into the canonical `/listings/{id}` page so users browse the marketplace and discover lobbies through the same flow they already know. **Slice A shipped 2026-06-12** (listing-card adapter on `/listings` + `/listings/mine` — 5v5 / 2v2 badge, fill counter, ready-check pulse, "View lobby" CTA, +5 tests). **Slice A.2 shipped 2026-06-12** (user-toggleable rows ↔ grid view backed by a `listings_view_layout` cookie + Inertia-shared prop, grid card with roster-preview avatars + fill counter, amber `Ready check · MM:SS` countdown banner at the top of team-play grid cards, +9 tests). **Slice B.1 shipped 2026-06-12** (canonical `/listings/{id}` hosts the lobby UI for team-play; legacy `/lobbies/{id}` 301-redirects; `TakeListingAction` `'not_takeable'` guard; `viewLobby` policy relaxed to URL-as-access). **Slice B.2 shipped 2026-06-12** — the FACEIT-grade center column landed: `LobbyResource.aggregates` block (pot/fee/winner/loser money math, per-team skill avg/min/max + delta tone, per-team trust avg completion + settled), 4 center-column React blocks (Money HERO / Skill matchup / Trust signals / state-dependent Coordination panel with countdown + click-to-copy FACEIT usernames), 3-col `1fr_minmax(360px,400px)_1fr` layout at lg+, +14 backend tests. Seeders now produce all three lobby states (recruiting / ready_checking / locked) end-to-end through real actions for full testability. **Polish round 1 shipped 2026-06-12** — CS2 5v5-only, view-toggle persistence sync, `/listings/mine` rows-only, lobby slot card respec (no glow, Team {leader} header, full-width empty slots), floating chat card. **Polish round 2 shipped 2026-06-12** (6 items) — Ready/Leave moved into the Money block under the platform-fee line (slot card stays display-only, deleted `lobby-actions.tsx`), Leave stays on lobby page (`back()` instead of `listings.index` redirect), chat FAB hidden for non-participants, slot stats reworked to Matches / Win rate / Completion-30d, recruiting Coordination panel dropped (team-column fill counters already carry the info), Ready/Leave recoloured to traffic-light tones (solid emerald + translucent rose, matching the win/lose tiles above). **Polish round 3 shipped 2026-06-12** — homepage "Ending soon" card unified with `/listings` grid card (deleted obsolete `ListingCard`, swapped to `ListingGridCard` everywhere). **Phase 4 shipped 2026-06-13** — FACEIT 5v5 verification: `isOpposingTeamRosters` strict-N-vs-N check (slot 0 + slot 1 fallback per team, ~44 calls worst case), `SettleTeamMatchAction` fan-out (5 payouts + 1 fee + slot-0 remainder + in-action conservation assertion), defensive winner-roster check in `SettleFromCardAction`, +9 tests. Locked CS2 5v5 lobbies now settle end-to-end through the same auto-fetch pipeline that powers chess. **CS2 production launch (M15) now functionally unblocked** — only Phase 5 (2v2 Wingman) remains for the team-play range. Phases 0–3 all shipped 2026-06-12 (1330 → 1410, +80 tests across the four phases).
