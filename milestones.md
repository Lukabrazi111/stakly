...
- **M34 — match roster-card polish (via `/design-review`).** *Built + verified in-browser 2026-07-05 (match #221); frontend-only; pending user commit.* The `matches/{id}` Rosters cards (`team-rosters.tsx`) were ragged (uneven heights), had a bare unlabeled grey rating chip, no team-color identity, said "You", non-clickable names, and a whisper-thin "No matches yet". Frontend-only pass (no backend): team-color avatar rings (A pink / B purple, mirrors the lobby), a labeled "Rating" block, name → profile link (plain-underline hover, real name not "You"), and the SAME 3-stat grid on **every** card (new players read `0 / — / —`) so all cards stay the same height. FACEIT dial + W/L-strip parity **SHIPPED 2026-07-06** (pending user commit): `TeamMatchPlayer` gained `faceit_rating` (dial object, derived from the platform ELO via `FaceitLevel::fromElo`) + `recent_form` (last-5 W/L, batched in `GameMatchController::show` via `RecentForm::forBatch` scoped to the listing game); `team-rosters.tsx` now reuses the shared `FaceitRatingBadge` (dial) + `RecentFormStrip` (flush right-edge column) so the match cards match the lobby slot cards at full parity. `TeamMatchResourceTest` shape assertions extended; browser-verified on `/matches/3` (dial + W/L strip render, players with no history show the quiet "—" column); tsc + build + Pint clean.
...
- [ ] M34 match roster ↔ lobby **parity** (FACEIT level dial + W/L strip) — `GameMatchController`, `GameMatchResource`, `team-rosters.tsx`, `types/match.ts`, `TeamMatchResourceTest`. Suggested: `feat: M34 — FACEIT dial + W/L strip on match roster (lobby parity)`.

**M34 — shipped + committed this session (2026-07-06):** (B) platform handles on match roster · "View lobby" link on match page · (A) locked-lobby→match redirect built-then-reverted (jarring on final-Ready) · **H2** lobby refund idempotency keys (kick/leave, row-id anchored).

**M34 — remaining to close out:**
- [ ] **#3 — country flags on lobby cards** — needs a `linked_accounts.country` column + FACEIT country fetch + backfill migration. The one real M34 item left after the parity work above.

**Security hardening — queued from `/cso` (2026-07-06):** _money-flow audit came back clean (0 CRITICAL / 0 HIGH / 0 exploitable). H2 shipped (above); H1 still queued:_
