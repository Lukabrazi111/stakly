...
- **M26 Phase 4** — Full-site i18n. Slices A/B/C ✓ 2026-06-04 + global `URL::defaults` fallback ✓ 2026-06-05. **Slices D-1 through D-11 (Listings + Profile + Match + Wallet + Notifications + Settings + Auth) ✓ 2026-06-05** — listings (D-1..D-5), public profile (D-6), match (D-7a..D-7e), wallet (D-8), notifications (D-9), settings (D-10), auth modal + page landings + Fortify responses (D-11). **Up next: D-12 BannedBanner / M30 user-facing → D-13 Validation/flash/errors**. Translation labor (`lang/ka.json` / `lang/ru.json` content) tracked separately as a content backlog.
