...
- **M42 — Filament admin audit fixes (multi-game + team-play + perf)** — *done 2026-06-30 — all 48 audit findings fixed + the Games-Active gate tightened (+ a discovered gate no-op); full backend suite **1691** green, pint clean.* A 55-agent adversarial audit of the whole admin panel (triggered by a user-reported `UnhandledMatchError` 500 on `/admin/listings`) surfaced **48 confirmed issues** from the M15 (FACEIT/Steam providers, Cs2/Dota2 games), M34 (team lobbies), and M41 (ratings) growth that the admin never caught up with: **4 crashes** (non-exhaustive `match()` over `LinkedAccountProvider` in `ListingsTable`/`ListingInfolist`; over `AutoFetchOutcome::AcIncomplete` twice in `GameMatchInfolist` — 500s the dispute page), **16 bugs** (dispute view is 1v1-shaped — no team roster, blank "Taker", money pot hardcoded `stake×2` not `×team_size×2`, provider handles read LIVE not the `match_provider_snapshots` evidence anchor + miss FACEIT/Steam; admin `ViewListing` force-cancel runs the 1v1 `CancelListingAction` on a team lobby → mis-refunds escrow; `UserInfolist` open-disputes use `forParticipant` so team roster members' disputes are invisible; `WalletReferenceParser` can't parse team settlement refs; `ProviderCircuitBanner` ignores FACEIT; …), **15 UX**, **7 perf** (widget COUNT-loops per 30 s poll; missing eager-loads). Fixing all 48: enum-owned labels/badges as the single source of truth, a Team A/Team B roster section sourced from `lobby_participants` + `match_provider_snapshots`, team-aware money, roster-scope queries, and wallet-trail grouping by `related_listing_id`. Full verified list in the audit output. Money-adjacent fixes (dispute view, force-cancel, wallet trail) done hands-on; supporting areas (widgets, CMS, games, user infolist, chat roles) via a parallel fix workflow. **Key fixes:** `AutoFetchOutcome` now owns its badge color + label (single source of truth — the AcIncomplete 500 can't recur); the dispute view has a **Team A / Team B roster** (live `lobby_participants` + at-lock `match_provider_snapshots`), team-aware money (pot = stake × team_size × 2 + per-player share), and provider handles read from the snapshot evidence anchor (all 4 providers); new `App\Actions\Lobby\Admin\ForceCancelTeamLobbyAction` (refunds every Ready player, vacates roster, cancels listing + match) wired as a separate team header action while the 1v1 force-cancel is gated to `! isTeamPlay()`; wallet-trail siblings now group by `related_listing_id`. **Caught during verification:** the wallet agent's literal idempotency-`reference` on the Ready/un-Ready toggle would have shorted the pot on a re-Ready (Wallet idempotency returns the prior hold, holds nothing) — reverted; grouping is via `related_listing_id` instead. New tests: `M42MultiGameAdminTest`, `ForceCancelTeamLobbyTest`, + extended parser/resource tests. **Games "Active" gate — decided + done (2026-06-30, with the user):** tightened to require `Game::hasArbitrationDriver()` (chess + cs2 ✓, dota2 ✗) in both `GameResource` + `Game::hasBackendIntegration()`, so an admin can no longer flip adapter-less Dota2 to Active. Implementing it exposed that the gate had been a **silent no-op**: `->rules([fn (Get $get) …])` (array form) hands the closure straight to Laravel, which calls it as a raw rule (`$get` = the attribute string, returned closure ignored) — switched to `->rule(…)` (Filament-injected); and the supporting-fix `->options(GameStatus::class)` change made `$get('status')` return the enum, so the `=== ->value` compare missed — normalized to accept enum-or-string. Now covered by `GameResourceGateTest` (Dota2/Valorant Active → slug error; CS2 Active ✓; Dota2 ComingSoon ✓). **1691 backend tests green.** Minor follow-up: the Games poster placeholder uses an external `placehold.co` URL (admin-only; swap to a local asset if offline-admin matters).

- **Match-page "Money breakdown" — device-aware hover/tap (2026-06-30).** The team-match money-breakdown pill (`match-details-strip.tsx`) was a Radix **HoverCard** (hover-only): on mobile it never opened on tap AND its trigger `preventDefault`'d `touchstart` in React's passive root listener → console spam *"Unable to preventDefault inside passive event listener invocation."* Now **device-aware** via a new `useHasHover()` hook (`matchMedia('(hover: hover)')`, SSR-safe `useSyncExternalStore` like `useIsMobile`): hover-capable devices keep the **HoverCard** (opens on hover/focus; snappier `openDelay=150`), touch devices get a **Popover** (opens on tap; no touchstart-preventDefault). Shared trigger + `BreakdownPanel`; `PopoverContent` gets the `w-80 rounded-xl p-5` className the HoverCard skin already carries so the edge-bleed "Verified via :platform" footer matches. Also simplified the hover affordance to a single subtle `bg-primary/10` wash (was a full border+bg+text+icon pink flip). Verified via Playwright: desktop hover opens the card, mobile tap opens the popover, console clean (0 errors). FE gates green.
