...
- **M34 — lobby refund idempotency keys (`/cso` H2, 2026-07-06).** *SHIPPED 2026-07-06 (pending user commit). Added a per-participation idempotency `reference` to the two TERMINAL lobby refunds — `KickParticipantAction` (`kick-refund:{participant_id}`) + `LeaveLobbyAction` single-leave & creator-cancel (`leave-refund:{participant_id}`). Anchored on the **LobbyParticipant row id**, NOT `{listing}:{uid}` — a kicked player can rejoin (new row) + be kicked again, and `{listing}:{uid}` would idempotency-block the 2nd refund and SHORT them their re-staked funds. **Excluded the un-Ready release by design** — it's a repeatable toggle (mirrors the documented `markReady` hold side); a static ref would short the pot on re-ready, so it stays reference-less, guarded by the row lock + `is_ready` flip. Parser left untouched: the refs carry a participant id (not listing/match), so `WalletReferenceParser` correctly returns null (no admin ref-link) — `wallet_transactions.related_listing_id` still ties the row to its listing, so no admin-display regression. Tests: `LobbyRefundIdempotencyTest` (5 — kick/leave/creator-cancel refs + rejoin-double-refund + un-Ready toggle stays live). Lobby 164 + Wallet 45 + parser/admin 43 green.* The three lobby escrow **release** paths — `KickParticipantAction.php:63`, `LeaveLobbyAction.php:97/125`, `ToggleReadyAction.php:110` (un-Ready) — call `Wallet::release(...)` with **no `reference`** idempotency key, unlike every other refund path in the codebase (`listing-cancel:{id}`, `match-draw:{id}:{uid}`, `cancel-refund:{id}:{uid}`). Safe *today*: all lobby actions serialize on the listing `lockForUpdate`, and each refund is gated by an immediate `is_ready`/`stake_held_at`/`kicked_at` state transition, so concurrent double-refund is impossible (traced end-to-end during the /cso audit). Risk is **latent** — a future refactor that weakens the row lock or the state guard turns this into a live double-refund. The `hold` side omitting a reference is correctly justified (Ready is a repeatable toggle — a static ref would short the pot on re-Ready); the fix is the *release* side only. **Scope:** add a per-event reference to the 3 releases (e.g. `kick-refund:{listing}:{uid}:{kicked_at}`, `leave-refund:{listing}:{uid}`, `unready-refund:{listing}:{uid}:{ts}`) + a Pest test asserting a double-kick / double-leave posts exactly one refund row. Money-touching → high-stakes mode. (Minor sibling notes from the same audit: assert `config('stakly.platform_fee_rate')` ∈ [0,1) at boot; add `.gstack/` to `.gitignore` before saving any `/cso` report locally.)
