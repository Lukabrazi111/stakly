...
## Phase 6.5 — Listings management UI + design polish **(in progress)** (~2 days, no new deps)
...
- [ ] **6.5.5** Tests — `/listings/mine` page renders, auth-gated, only your listings visible, Listed vs All Ads tabs scope correctly, pagination works, max-2 cap enforced server-side, Active Mode toggle endpoint (POST `/active-mode`) flips state + auto-rejects same-state submissions silently, marketplace + public profile both hide listings when owner is inactive, Tools dropdown links resolve.
