...
- **M34 — kick-button placement (open design question).** The slot-card kick ✕ (top-right, `slot-card.tsx`) overlaps the FACEIT level dial; several redesigns were tried and reverted, so it's back to the original ✕. Revisit with fresh direction. (M34 core + the 2026-06-28/30 private-lobby-access + invite-share hardening all shipped — archived.)
