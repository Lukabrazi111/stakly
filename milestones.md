...
- **M34 Phase 3.1** — Unify lobby into the listing page. Frontend follow-up to P3: collapse the dedicated `/lobbies/{id}` URL into the canonical `/listings/{id}` page so users browse the marketplace and discover lobbies through the same flow they already know. **Slice A shipped 2026-06-12** (listing-card adapter on `/listings` + `/listings/mine` — 5v5 / 2v2 badge, fill counter, ready-check pulse, "View lobby" CTA, +5 tests). **Slice A.2 shipped 2026-06-12** (user-toggleable rows ↔ grid view backed by a `listings_view_layout` cookie + Inertia-shared prop, grid card with roster-preview avatars + fill counter, amber `Ready check · MM:SS` countdown banner at the top of team-play grid cards, +9 tests). **Slice B.1 shipped 2026-06-12** (canonical `/listings/{id}` hosts the lobby UI for team-play; legacy `/lobbies/{id}` 301-redirects; `TakeListingAction` `'not_takeable'` guard; `viewLobby` policy relaxed to URL-as-access). **Slice B.2 shipped 2026-06-12** — the FACEIT-grade center column landed: `LobbyResource.aggregates` block (pot/fee/winner/loser money math, per-team skill avg/min/max + delta tone, per-team trust avg completion + settled), 4 center-column React blocks (Money HERO / Skill matchup / Trust signals / state-dependent Coordination panel with countdown + click-to-copy FACEIT usernames), 3-col `1fr_minmax(360px,400px)_1fr` layout at lg+, +14 backend tests. Seeders now produce all three lobby states (recruiting / ready_checking / locked) end-to-end through real actions for full testability. **Polish & fixes pass next** (in-flight section below). P4 (FACEIT 5v5 verification) is the next milestone after — without P4, locked CS2 lobbies fall to ManualReview after the 4h timeout. Phases 0–3 all shipped 2026-06-12 (1330 → 1410, +80 tests across the four phases).
...
**Phase 3.1 Slice B — Polish & fixes (in-flight)**

In-flight tweaks after the B.2 lands. The user walks through the page and we fix things step-by-step rather than designing the whole follow-up upfront. Captures the visual / UX gaps spotted in dogfooding before they get lost.

- [ ] **CS2 is team-play only — lock `team_size = 5`, no 1v1 CS2 listings.** Marketplace currently shows mixed CS2 1v1 ("Take" CTA) + CS2 5v5 ("View lobby" CTA) listings because `Game::Cs2->allowedTeamSizes()` still accepts `[1, 5]`. FACEIT CS2 is 5v5; 1v1 doesn't exist on our platform. Tighten the enum to `[5]`, update `StoreListingRequest` validation, drop the 1v1 CS2 branch from `ListingSeeder` so every CS2 row on `/listings` is a 5v5 lobby with a `View lobby` CTA. Reserve the `[2]` Wingman slot for M34 P5 if we ship it.
- [ ] **View toggle (rows / grid) doesn't persist between navigations.** User clicks a layout, navigates away, comes back, the OTHER layout is active. Most likely cause: `useState(initial)` in `useListingsView` captures the cookie-backed prop value on first render only and doesn't re-sync when Inertia hands back a new shared prop after navigation. Secondary suspect: stale encrypted cookie from before the `bootstrap/app.php` encryption-exception fix is still in the browser jar. Fix: (1) `useEffect` syncing local state to the shared prop, (2) belt-and-braces — server clears any unparseable cookie via `Cookie::forget` so stale state self-heals, (3) verify in DevTools that `document.cookie` write actually persists across navigations.
- [ ] **`/listings/mine` keeps only the rows layout — no grid toggle.** The owner dashboard is a comparison-first management surface; the grid card's roster-preview + money block aren't useful for the user's own listings. Drop `<ListingsViewToggle>` from `pages/listings/mine.tsx`, render `<MineListingRow>` unconditionally, delete `mine-listing-grid-card.tsx` + its skeleton.
- [ ] **Lobby page — visual polish vs reference `images-example/lobby.png`.**
  - Drop the magenta glow halo from filled slot cards. The reference uses a flat bordered card with a subtle highlight on Ready, not the `shadow-(--shadow-arena-card-glow)` we currently apply on `is_ready`.
  - Rename team headers from `Team A` / `Team B` → `Team {leaderUsername}`. Leader = creator on creator's side, first-joined participant on the opposing side. Mirrors how the reference labels teams.
  - Re-spec the slot card per the cleaner reference (avatar top-left, username + platform name in the middle, rating chip on the right — better spacing, less nested-flex chrome). Currently the layout reads cluttered.
  - **Empty slot ("Join Team A") cards must be full-width within their column.** Currently they render at a narrower intrinsic width than the filled slots so the column feels broken. Force `w-full` on the empty-slot button.
  - Per-slot card adds an **Overall · Last 20 matches** stats line (matches, win rate). Source = our own platform's match-history (already exposed on the public profile — total matches + win rate + completion %). NOT FACEIT's `/players/{guid}/stats` — keep the lobby page off the FACEIT Data API per the deferred per-player W/L decision. May need a small stats block in `LobbyResource.presentParticipant` computed via a batched query similar to `SellerTrust::forBatch`.
- [ ] **Chat — floating window, not a full-screen Sheet.** Currently `MobileChatTrigger` opens a Radix `Sheet` which on desktop slides in covering most of the right edge. The user wants a compact floating chat window pinned to bottom-right that expands inline (≈380 px wide × ≈420 px tall) on click, NOT a full-height drawer. Reference shape: `floating-chat.txt` in repo root — `motion` enter/exit, rounded card, chat-area scroll, input row at the bottom. Reuse our existing chat plumbing (`useMatchChat`, the existing message-list / input pieces) — only the OUTER shell changes from Sheet → floating motion card. Strip the agent-selector `Select` + AI-agent presets from the reference snippet — those are demo UI we don't need.

Once a fix lands the checkbox flips to `[x]` with a one-line note on what shipped. New items get appended as the user surfaces them.
