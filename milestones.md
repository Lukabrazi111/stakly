...
- **M46 — Harden chess result auto-detection (TOP PRIORITY — planning done 2026-07-07: office-hours + eng-review)** — keep the after-the-fact API watch-and-search model, but make it reliably land the right chess game on its own (tighter disambiguation, archive-lag / Lichess-stream handling, clearer player guidance) so results come from the API without false `ManualReview`s. **Chess first;** CS2/FACEIT result verification is now its own milestone **M47 (below).** No game orchestration, no URL-paste for chess (both parked unless hardening proves insufficient). Audit-first — full section below.
...
## M46 — Harden chess result auto-detection (verified match results)

> **TOP PRIORITY. Planning complete 2026-07-07 (`/office-hours` + `/plan-eng-review`). Build order: P2 correctness now (provable with tests), P1 audit = instrument now + tune post-launch on real traffic.**

**Problem.** Stakly settles matches from the game provider's API, never from player self-report — good. But it does so by *watching after the fact*: a background job searches the provider's history for "a game between these players around this time" (`AutoFetch*GameJob` → `searchGamesBetween`). That search is probabilistic and routes to `ManualReview` whenever it can't cleanly identify the staked game — multiple games in the window, wrong time control, archive lag, abandoned / aborted games, opponent-not-found. Those false `ManualReview`s are the fragility.

**Direction (chosen 2026-07-07).** Keep the existing watch-and-search model — do **not** add game orchestration (Stakly creating the Lichess game) or URL-capture (player pastes the game link). Instead **harden the automated search for chess** so it reliably lands the right game on its own. **Chess first; CS2/FACEIT deferred** — the hard part of CS2 is coordinating 10 players into one attributable FACEIT match, and FACEIT's read-only API + provider-side team assignment make that a separate, larger problem for later.
...
**▶ Phase status:** P1 ⏸️ (deferred → post-launch: audit is meaningless without real traffic) · **P2 ✅ DONE (2026-07-07, committed)** · **P3 ✅ DONE (2026-07-07)** · **P4 ✅ DONE (2026-07-07)** · **P5 ✅ DONE (2026-07-07)**
**▶ M46 COMPLETE** except the post-launch P1 audit (needs real production traffic to tune). All buildable chess-hardening phases (P2–P5) shipped 2026-07-07 on branch `feat/m46-chess-disambiguation`.

**`/review` pass on the branch (2026-07-07, gstack — 4 specialists + adversarial, Codex unavailable):**
- **[SECURITY, real fix] The P2 started-after guard was a NO-OP on chess.com.** The adversarial pass (verified against the LIVE chess.com API — 78 real blitz games) found `parseGame` derived `createdAt` from `start_time ?? end_time`, but chess.com live games (all Stakly chess.com TCs) carry **no** `start_time` — only daily games do. So `createdAt` fell back to `end_time` and `rejectStale` collapsed to the archive search's own `end_time >= since` filter: a game already in progress when the stake was placed could auto-settle. **Root cause hidden by the test fixture**, which fabricated a `start_time` on a blitz game the real API never returns. **Fixed:** `ChessComGameClient::parseStartTimestamp()` now parses the true start from the PGN `[UTCDate]`+`[StartTime]` (UTC, confirmed), `end_time` floor only if unparseable; also corrects the pick-ordering (was earliest-*ended*, now earliest-*started*). Fixture rewritten to the real shape (start in PGN, no top-level field). +client PGN-parse proof + `end_time`-fallback + mixed stale+real composition tests (chess + Lichess). Lichess was always sound (real `createdAt`). 272 Jobs+Provider tests green.
- **[perf, fixed] N+1 in the cron loop** — `AutoFetchPendingMatches` now `->with(['listing','providerSnapshots'])` (was ~200 queries/chunk, amplified 5× by the P3 young tier).
- **[perf, fixed] Page-visit write amplification** — `GameMatchController::show()` now gates the auto-fetch dispatch on `Pending`, so viewing a resolved match no longer writes a `not_pending` skip row every time (was unbounded `match_auto_fetch_attempts` growth; P5's skip-ignoring summary was a workaround for this).
- **[maint, fixed] Copy branch** — `waiting-for-game-card` discriminates on `timeControl !== null`, not the formatter's empty string.
- **Deferred (user call):** extract the now-identical retry-tail helpers from the two auto-fetch jobs into a shared trait — revisit when FACEIT adopts the no_match chain (M47). Consistent with the sibling-mirror convention.
- Clean: SQL/enum/injection/XSS/idempotency all verified; the P5 admin surfaces + coarse-safe player banner confirmed sound.

**P3 accepted trade-off (5-lens adversarial review, 2026-07-07 — no blockers/highs, change confirmed latency-only):** the young cron is provider-agnostic, so during a **Lichess stream-sidecar outage** each stuck [1-10min] Lichess match makes ~4 export calls/min; the shared `lichess-api` 60/min bucket saturates at **~15 concurrent stuck matches** → degrades to added latency (NOT a breaker trip / settlement freeze — rate-limit releases happen in middleware before the breaker's catch blocks; chess.com is a separate bucket). Safe at launch scale; revisit (per-match rate-limit key, or make the young sweep chess.com-only) before high Lichess volume.
...
- **P3 — Window + retry + stream hardening. ✅ DONE (2026-07-07, 5-lens adversarial review passed — no blockers/highs; +9 Pest tests incl. chain-boundary pins; 83 auto-fetch/cron/stream tests green + Pint clean).** **Scope (user): adaptive job-retry tail + young-match cron tier.** _Confirmed first (per eng-review):_ the Lichess real-time stream is **already fully wired** end-to-end — `LichessStreamCommand::dispatchFromEvent` finds the Pending match by both player IDs (platform-scoped) on each game-end event and dispatches `AutoFetchLichessGameJob`; sidecar runs in `compose.yaml` (`restart: unless-stopped`). So "lean on the stream" = harden the job to *catch* that signal, not new wiring. **This is a latency-only phase — no correctness/security change** (P2 stale-guard runs on every attempt; the 4h M16 timeout + cron backstop already guarantee eventual settlement, so NoMatch never directly → ManualReview). Changes: **(a) chess.com** — extend the no-match retry tail `[5,15,45]` → `[5,15,45,90,150]` (~5 min, 6 attempts) so a game played within ~5 min of the take settles via the job's own chain instead of waiting for the 10-min cron; `uniqueFor` 360→600 to cover the longer chain + rate-limit headroom (capped at the 10-min young-window boundary). **(b) new young-match cron tier** — `stakly:auto-fetch-pending --min-age-minutes=1 --max-age-minutes=10` @everyMinute, complementing the existing `[10min,4h]`@5min backstop, closing the [chain-exhaust → 10-min-cron] re-dispatch seam (cuts typical chess.com settlement ~10-15 min → ~1-2 min). **(c) Lichess** — add a short no-match retry chain `[3,8,20]` (~31 s, 4 attempts) to bridge Lichess game-export indexing lag so the fast stream signal lands on the first shot instead of falling through to cron. Young cron tier is **provider-agnostic** (reuses the existing command) so it also backstops a Lichess stream-sidecar outage; jobs stay `ShouldBeUnique` + `RateLimited` + circuit-breaker-guarded so the finer cadence can't hammer a provider.
- **P4 — Player guidance UX. ✅ DONE (2026-07-07).** Folded the precise instruction into the existing Pending-state `WaitingForGameCard` (no redundant second card): for chess 1v1 the heading now reads **"Play your rated {Blitz} game on {chess.com}"** + body **"Start it now and play a single game to a finish — Stakly reads the result automatically, so there's nothing to report here."** — carrying all four constraints the finder keys on (**rated · exact time control · one game · started now**); the existing "Watching for {A-handle} vs {B-handle}" box supplies the opponent/account anchor. **Pure frontend** — every needed field (`listing.time_control`, `platform`, snapshots) was already on `GameMatchResource`; only `waiting-for-game-card.tsx` (+ new `timeControl` prop) + its two callers (`show.tsx`, `team-match-view.tsx`) changed. Copy **branches on time-control presence**: null (CS2/team) keeps the original generic copy — CS2 setup guidance is M47 P2's job, not duplicated here. Typecheck + Prettier green. (Design skills `ui-ux-pro-max` + `frontend-design` applied — restraint: enhanced the one existing card rather than adding decoration.)
- **P5 — Residual ambiguity → cleaner ManualReview. ✅ DONE (2026-07-07).** Display-only clarity (M42 already built the resolve actions + audit + Wallet-transactional payout — P5 touches ZERO money code). **Admin:** shared `MatchAutoFetchAttempt::reasonLabel()` map turns the raw `outcome_reason` codes into plain English in the timeline (`retry_exhausted` → "No game found after all retries", `stale_game_rejected` → "Found a game, but it started before the stake", etc.; unknown codes humanised, never raw snake_case) + a new **"Why it's in review"** infolist section (first thing shown on a ManualReview/Disputed match, hidden otherwise) synthesising the terminal auto-fetch attempt into one sentence. **Player:** `admin-review-banner.tsx` now branches the `manual_review` copy on the existing `dispute.opened_by_id` — accurate timeout-vs-dispute wording (fixed a real mislabel: it used to always say "confirmation window expired" even for a dispute that couldn't be auto-resolved) + "what happens next / stake in escrow / post evidence". **[SECURITY, user sign-off]** players get only the COARSE reason (timeout vs dispute) — the granular detection reasons stay ADMIN-ONLY so the anti pre-play-reuse guard (`stale_game_rejected`) is never leaked to a would-be cheater. +9 Pest (reason-label unit map + "why in review" shown/hidden render tests; 2 existing admin render assertions updated raw→label; DB-column assertions untouched = write path unaffected); 41 admin + 24 reason-data tests green, Pint + types:check + Prettier clean. **Fix (same session, caught in dogfooding):** the "Why it's in review" summary keyed off the LATEST auto-fetch attempt, but `GameMatchController::show` dispatches auto-fetch on every match-page view and a non-Pending match records a `not_pending` **skip** row each time → the summary flipped to "skipped — no longer pending" on refresh. Now it picks the last **non-Skipped** (real detection) attempt, falling back to the last skip only if nothing ever reached the provider (e.g. `snapshot_missing` throughout). +2 stability/fallback render tests. (Root-cause noise — the page-visit dispatch writing skip rows for already-resolved matches — left as-is; optional follow-up to gate that dispatch on Pending.)

**Eng-review hardening (2026-07-07 via `/plan-eng-review`):**
- **[SECURITY — money] The "started-after-match-creation" rule (P2) is a security control, not just disambiguation.** The current job disambiguates by *closest-to-`created_at`* + *ended-after-`since`* — which can pick a game that STARTED before the stake existed. Without the started-after guard, two players who already played (a game before the listing, or one in progress at listing time) could have it auto-detected + settled = a pre-play attack. Make **"first decisive game that STARTED after match creation, matching the stake TC"** the PRIMARY rule; must-test. (Same class as M47's stale-match guard — both caught this session.)
- **[Sequencing] Build P2 now; the P1 audit needs real traffic.** P1's real value (what breaks most) only shows against production data — dev is seeded / fake. So in dev, P1 = instrument + validate the query + categorization; the real distribution comes post-launch to tune P3/P4. P2's correctness items (started-after guard, exact TC, deterministic pick) are provable NOW with crafted tests — don't block them on the audit.
- **[Lichess stream — confirm before wiring] (P3).** Read `LichessStreamCommand` FIRST to see what it covers today. If it already streams the two players' games, wiring game-end → trigger auto-fetch is a fast win (near-instant Lichess settlement vs archive-poll lag). chess.com stays archive-poll-laggy (no stream) — inherent; just don't give up too early (premature NoMatch → false ManualReview).
- **[Observability] (P1/P5).** Add a distinct `stale_game_rejected` outcome_reason in `match_auto_fetch_attempts` (found a game but it started before match creation) so the audit separates "found-but-stale" from "no-game-found."
- **[Collusion = ACCEPTED risk].** Two players agreeing a result off-platform — the API faithfully reports it; detection can't stop it. Behavioral trust signals later, out of M46 scope.
- **[Pest matrix — P2]:** game-started-after-creation enforced; game-started-BEFORE-creation → rejected (not settled); exact-TC match; first-decisive-game pick among a rematch; abandoned → refund; draw → refund both; idempotency (double auto-fetch → single settle); `usdt_balance == SUM(wallet_transactions)` holds.

**Deferred (not this milestone):** CS2 / FACEIT result verification is now its own milestone → **M47 (below).** Lichess game orchestration + URL-capture for chess — parked unless hardening proves insufficient.
...
- **M46 (chess):** P1 ⏸️ (deferred → post-launch: audit needs real traffic) · **P2 ✅ DONE + committed** (started-after-creation SECURITY guard + deterministic pick + `stale_game_rejected`; branch `feat/m46-chess-disambiguation`) · P3 ⬜ · P4 ⬜ · P5 ⬜.  **→ Next: M46 P3 (Lichess-stream / retry hardening) or P4 (player-guidance UX)** — both buildable now.
