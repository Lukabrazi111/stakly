...
- **P3 — Window + retry + stream hardening.** Better archive-lag handling for chess.com; adaptive retry windows; lean on the existing Lichess real-time stream (`LichessStreamCommand` / `lichess-stream` service — confirm current coverage) for faster, cleaner Lichess detection vs archive polling.
