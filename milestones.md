...
- **M34 — match/lobby consolidation + FACEIT handle on match roster.** *Planned (decided 2026-07-05 via `/design-review` on `matches/221`; build next session).* Two decisions taken together:
...
_Living "you are here" pointer — the short version of what to work on right now. Update as work lands; full detail lives in the milestone entries above. Last updated 2026-07-05._

**Uncommitted right now — commit these:**
- [ ] Match roster-card polish (`team-rosters.tsx`) + this file → `style: M34 — match roster cards (team rings, labeled rating, profile links, uniform heights)`

**Build next — M34 match/lobby consolidation** _(full scope + open questions in the M34 entry under "## Phases (map)" above):_
- [ ] **(A)** Once a team lobby LOCKS, redirect `/listings/{id}` → `/matches/{id}` (match = single canonical post-lock page). **Resolve FIRST:** can a non-participant view a PUBLIC match, or would they 404 on the redirect? (`GameMatchPolicy::view`.) Then Pest tests: locked→redirect, recruiting/ready_checking→no-redirect, private-listing 404 unchanged.
- [ ] **(B)** Add each player's FACEIT/chess **platform handle** to the match roster (from `providerSnapshots` → `GameMatchResource`/`TeamMatchPlayer` → `team-rosters.tsx`) for in-game scouting. Small backend + resource-shape test. _(W/L strip stays dropped.)_
