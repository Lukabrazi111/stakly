...
**Open follow-up (small):**

- **Chat content leak on the team-play lobby page.** `ListingController::showTeamPlay` always ships up to 200 `messages.data` to the Inertia payload regardless of viewer participation. Non-participants can read chat from the page JSON. Fix: gate the `$messages` build on `auth()->user()` being a live participant; ship empty collection otherwise. ~5 lines. Same boundary as the chat-subscription gate just shipped in P3.2.
