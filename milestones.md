...
- **M30** — Admin user management (Filament `UserResource` — search, view, manual email verify, reset 2FA, ban toggle with real enforcement, mandatory 2FA on admin role, user-facing ban feedback, custom impersonation flow, 2FA challenge on every admin login; closes the biggest support gap in the admin panel + hardens admin access). **Phases 1 + 2 + 3 + 4 shipped 2026-06-03. Phases 5 (impersonate) + 6 (admin 2FA challenge on every login, bridge to Fortify) remain.** <- (in process)
...
- **View page — stacked sections.** Profile summary (avatar, name, username, bio, member-since), linked accounts (provider + verified date), wallet snapshot (balance + lifetime fees paid + last deposit + inline invariant-verify button), username history (released handles from `username_history` + reservation expiry), recent listings (last 10 with state badges), recent matches (last 10 with status badges), open disputes the user is party to. Inline Infolist sections — matching `GameMatchInfolist`'s precedent of rendering audit data inline rather than via Relation Managers.
- **Inline wallet invariant-verify button.** Recomputes `SUM(wallet_transactions WHERE user_id = X)` and compares to `users.usdt_balance`. Renders a success badge ("Balance matches ledger") or a danger badge with the drift amount. Read-only; no confirmation. Should always pass — exists to catch a regression early when investigating a problem user.
- **Manual email verify action.** Flip `email_verified_at` to now. No reason required — low-impact + recoverable support scenario.
- **Reset 2FA action.** Clear `two_factor_secret` / `two_factor_recovery_codes` / `two_factor_confirmed_at`. No reason required — low-impact, user re-enrolls on next login.
- **Ban toggle action — reason required in both directions.** Adds `users.banned_at` (nullable timestamp) + a Filament toggle that flips it. Required reason on the confirm modal for BOTH directions — initial ban captures *why*; lifting captures *why now*. Audit value: months later we want a single trail of "what happened" without cross-referencing.
...
- **Ban-rejection flashes include a path to support.** Every guarded action's rejection message ends with a link to `/en/support` ("Account suspended — contact support if you believe this is a mistake"). Lightweight surface so users banned by mistake have a way to reach the dev without a full polished suspension page. Depends on the CMS Support page being published (M26 P2 seeded it as a draft); admin publishes it before M30 P2 ships in production. M21's polished suspension page formalizes the appeals UI more designed; until then, the flash + CMS Support page covers it.
- **"View as visitor" header action.** Quick link from the admin user page to `/users/{username}` (opens in a new tab). Mirrors `GameMatchResource`'s existing "Open as participant" pattern. Useful pre-step before deciding to impersonate — see the user's public profile from a visitor's perspective.
...
- [x] Migration: `users.banned_at` (nullable timestamp).
- [x] Migration: `admin_impersonations` table — `id`, `admin_user_id` FK, `target_user_id` FK, `reason` (string, max 1000), `started_at`, `ended_at` nullable, `ip_address` (string 45), `user_agent` (string 255). Append-only — `UPDATED_AT = null`, only `ended_at` mutates on exit.
- [x] `app/Filament/Resources/Users/` folder layout matching project convention: `UserResource.php`, `Pages/ListUsers.php`, `Pages/ViewUser.php` (stub for P2), `Schemas/UserInfolist.php` (stub for P2), `Tables/UsersTable.php`.
- [x] Navigation group: "Operations" (alongside `GameMatchResource`). Plural label "Users", model label "User", slug `users`.
- [x] `Tables/UsersTable` — search across `username` / `name` / `email`. Filters: verified email yes/no, banned yes/no, has-2FA yes/no, has-active-listing yes/no. Sortable by `created_at` / `usdt_balance` (settled_lifetime sort deferred to P2 once `SellerTrust` wiring lands on the table). `modifyQueryUsing` excludes `is_platform = true`.
- [x] `UserResource::canCreate()` returns false — admin doesn't manually create users.
- [x] Pest tests: admin-only access (non-admin → 403 / redirect), `is_platform` hidden from index, each filter resolves, search hits each column, default sort newest-first, no /create form.

Bonus: `AdminImpersonation` model also landed in P1 (was originally scoped to P4) — cleaner to add the relations + `scopeActive()` alongside the migration.

**Phase 2 — View page + non-impersonate actions + ban enforcement** ✅ Shipped 2026-06-03

- [x] Schema: `user_moderation_logs` (append-only audit log of ban / unban actions). `UPDATED_AT = null`; columns `user_id`, `admin_user_id`, `action` (`ban` | `unban` string), `reason` (string, max 1000), `created_at`.
- [x] `App\Models\UserModerationLog` with `ACTION_BAN` / `ACTION_UNBAN` constants + `user()` / `admin()` BelongsTo.
- [x] `Schemas/UserInfolist` sections: Profile (avatar / name / username / email / verified / 2FA / status badge / member-since / bio), Linked accounts, Wallet snapshot, Username history, Recent listings (last 10), Recent matches (last 10), Open disputes. Inline HTML where Filament can't compose (mirrors `GameMatchInfolist::autoFetchSummary`). Each data-dense section uses `->visible()` so the page stays clean for users with no activity.
- [x] Wallet snapshot — always-rendered invariant check (no button needed). Shows balance, per-type sums (Deposit / Hold / Release / Payout / Fee / Withdrawal), and a green/red badge comparing `users.usdt_balance` to `SUM(wallet_transactions.amount)`. Recomputes on every page load.
- [x] `ViewUser` header actions:
    - [x] View as visitor — link to `route('users.show', $record)`, opens in a new tab. No confirmation.
    - [x] Manual email verify — no reason, confirm modal, flips `email_verified_at = now()`. Hidden when already verified.
    - [x] Reset 2FA — no reason, confirm modal, clears `two_factor_secret` / `two_factor_recovery_codes` / `two_factor_confirmed_at`. Hidden when 2FA not enrolled.
    - [x] Ban toggle — reason required (Textarea, max 1000) in BOTH directions, confirm modal, flips `banned_at` between `null` and `now()` inside a DB transaction that also writes the `user_moderation_logs` row. Label, color, icon, modal heading, modal description, and prompt copy all flip per current state.
- [x] `App\Support\BanGuard` helper — single source of truth for `isBanned()`, `rejectionMessage()`, `supportUrl()`. Used by every guard so the flash copy stays consistent across the four enforcement surfaces.
- [x] Ban enforcement guards:
    - [x] `ListingController::create` — `Inertia::flash('toast', ...)` + redirect to `route('listings.index')` for banned users.
    - [x] `ListingController::store` — same short-circuit.
    - [x] `ProfileController::update` — same short-circuit (redirects to `route('profile.edit')`). Chose the controller-level guard over `ProfileUpdateRequest::authorize() === false` because the latter throws 403 with no flash; controller-level redirect is the cleaner UX.
    - [x] `User::usernameChangeBlockers()` — `'banned'` is the first blocker key (beats cooldown + in-flight match). `ProfileUpdateRequest::blockerMessage()` + `ChangeUsernameAction::assertNotBlocked()` both emit `BanGuard::rejectionMessage()` for that key.
    - [x] `Listing::scopeOnPublicMarketplace()` — chained `whereNull('banned_at')` on the creator. Banned users' listings vanish from the marketplace + visitor profile views (own profile still sees them, matching Active Mode's pattern).
- [x] Pest tests: 14 new (`tests/Feature/Admin/UserResourceActionsTest.php` for 8 action tests + `tests/Feature/BanEnforcementTest.php` for 6 guard tests). Full suite 1003/1003 green.

Two small calls flagged in P2 ship report:
- Skipped the parallel `banned-user store-listing` test — the create-form test already proves the controller-level guard short-circuit; a parallel store test would just maintain a valid stake/platform/time-control payload to reach the same `if (banned)` branch.
- Switched flash style from `->with('toast', ...)` to `Inertia::flash('toast', ...)`. The former was tripping Laravel's `assertRedirect` session-error inspection.

**Phase 3 — Mandatory 2FA for admin role** ✅ Shipped 2026-06-03

- [x] New middleware `App\Http\Middleware\RequireAdminTwoFactor` — for users with `admin` Spatie role, redirect to `route('security.edit')` when `two_factor_confirmed_at IS NULL`. Flash via `Inertia::flash('toast', ...)`. Non-admin users pass through to Filament's own `canAccessPanel` 403.
- [x] Registered in `AdminPanelProvider::authMiddleware()` alongside `Authenticate::class` — runs after auth, before Filament panel pages.
- [x] No new schema (Fortify columns + `TwoFactorAuthenticatable` trait already in place).
- [x] `UserFactory::admin()` now stamps `two_factor_confirmed_at` by default via `state()`. Existing tests that do `User::factory()->admin()->create()` keep passing without per-test 2FA wiring. To exercise the redirect path, override with `->create(['two_factor_confirmed_at' => null])`.
- [x] `AdminUserSeeder` stamps `two_factor_confirmed_at = now()` so local dev + CI don't trip the gate. Production deploy checklist should note: operator re-enrolls real 2FA via Settings → Security after first login.
- [x] Pest tests (5): admin without 2FA → 302 to `security.edit`; admin with 2FA → 200 on `/admin`; non-admin without 2FA → 403 (Filament's own gate, not bounced to security); the redirect target itself is then bounced through `password.confirm` (Fortify's existing flow, documenting the full chain); seeder stamps the column.

Gotcha: `/settings/security` carries Fortify's `password.confirm` middleware (since `confirmPassword: true` in `config/fortify.php`). The real admin flow is `/admin → /settings/security → /user/confirm-password → enter password → /settings/security → enrolls 2FA`. The test asserts the bounce so the chain is documented.

**Phase 4 — User-facing ban feedback (banner + in-app notification + email) — shipped 2026-06-03**

The P2 ban shipped enforcement (banned users can't act) but only a generic flash on guarded actions. P4 closes the explanation loop across three channels so the banned user can't miss what happened.

- [x] `BannedBanner` at `resources/js/components/site/banned-banner.tsx`. Mounted in `SiteLayout` above `SiteHeader`. Destructive tone, non-dismissible. Renders only when `auth.user.ban !== null`. Shows headline + reason + a `mailto:support@stakly.com?subject=Account%20suspension%20appeal` CTA. (Banner is not sticky — sticky header sits below it; persistent in-app bell + email back it up if user scrolls past.)
- [x] Expose `auth.user.ban` ({reason, banned_at}) via `HandleInertiaRequests::share()`. `latestBanLog` `HasOne` relationship on User uses `latestOfMany()` to pull the latest `action = 'ban'` row. Lazy-loaded only when `banned_at !== null` — unbanned users skip the join entirely.
- [x] `App\Notifications\AccountBanned` extending `PlayerNotification`. Overrides `via()` to return `['database', 'broadcast', 'mail']` unconditionally (bypasses parent's preference flow because moderation can't be silenced). Payload: `event_type = 'account_banned'`, title, body, `action_url = route('home')`, `related_id = moderation_log_id`.
- [x] `App\Notifications\AccountRestored` — symmetric class for the inverse. Same `via()` override.
- [x] Mail channel on both notifications via `MailMessage` (Laravel's default `notifications::email` Blade layout). Banned mail: subject "Your Stakly account has been suspended" + reason + paragraph on what's restricted + appeal CTA to `support@stakly.com`. Restored mail: subject "Your Stakly account has been restored" + "Open Stakly" action button. Queued via `ShouldQueue` (parent class) so ban / unban actions return fast even if SMTP is slow.
- [x] Dispatch from `ViewUser`'s `banToggleAction` AFTER the DB transaction commits. Same dispatch site for both ban and unban (branched on the `action` variable). Notification call sits outside the transaction closure so a queue/notification failure doesn't roll back the moderation write.
- [x] Real-time banner — when the `account_banned` / `account_restored` broadcast arrives at the user's Reverb channel, the existing `NotificationProvider` triggers `router.reload({ only: ['auth'] })` so the banner appears/disappears without a manual refresh.
- [x] Mandatory-channel treatment via the `via()` override (not via parent's `MANDATORY_EVENT_TYPES` list, which is reserved for money-affecting events that can't be opted out *in the bell*; moderation is mandatory across *all three* channels). `account_banned` / `account_restored` are not in `CONFIGURABLE_EVENT_TYPES`, so they don't appear in `/settings/notifications` — there's no opt-out to render.
- [x] Pest tests at `tests/Feature/Admin/BanNotificationTest.php` — 9 cases, 44 assertions: notification dispatch on ban + unban, channel coverage (database + broadcast + mail), mail subject + body content, Inertia share for banned vs not-banned users, ban → unban → ban chain (banner reads latest BAN reason). Full suite green (1017 tests).

Implementation deviations from the original spec, captured for audit:
- Class names: `AccountBanned` / `AccountRestored` (cleaner — namespace already says `Notifications`) instead of `UserBannedNotification` / `UserUnbannedNotification`.
- Inertia share key: `auth.user.ban` (nested object grouping `reason` + `banned_at`) instead of a flat `auth.user.banned_reason`.
- Support CTA: `mailto:support@stakly.com` per user decision (defer dedicated `/support` page to M21) instead of an in-app link.
- Mail templates: Laravel's default `notifications::email` Blade layout via `MailMessage->greeting()->line()` instead of custom `resources/views/emails/*.blade.php` files. Lighter touch — branded HTML email layout is its own polish pass (revisit when more transactional mail surfaces ship).
...
**Phase 6 — Admin 2FA challenge on every login (bridge to Fortify)**

- [ ] Custom Filament login page `App\Filament\Pages\Auth\AdminLogin` extends `Filament\Pages\Auth\Login`. Overrides `authenticate()` — after `parent::authenticate()` succeeds, if `auth()->user()` has `admin` role AND `two_factor_confirmed_at !== null`, stamp `admin_2fa_required = true` in session and return a redirect to `route('admin.two-factor-challenge')` instead of Filament's default panel redirect. Wired via `->login(AdminLogin::class)` in `AdminPanelProvider`.
- [ ] Routes (outside Filament panel, in `routes/web.php` under `auth` middleware): `GET /admin/two-factor-challenge` → `Admin\TwoFactorChallengeController@show` (returns `Inertia::render('admin/two-factor-challenge')`). `POST /admin/two-factor-challenge` → `@store` (validates code, sets session marker, redirects). `POST /admin/two-factor-challenge/cancel` → `@cancel` (logs out, redirects to home — escape hatch if admin loses TOTP device).
- [ ] `Admin\TwoFactorChallengeController@store` — validates `code` (TOTP) OR `recovery_code` (one-time use) via Fortify's existing `Laravel\Fortify\TwoFactorAuthenticationProvider::verify($user->two_factor_secret, $code)`. Recovery code path: match against `$user->recoveryCodes()` accessor, remove with `$user->replaceRecoveryCode($code)`. On success: clear `admin_2fa_required`, stamp `admin_2fa_passed = true`, redirect to `/admin`. On failure: redirect back with `errors`.
- [ ] Middleware `App\Http\Middleware\EnsureAdminTwoFactorChallenge` — registered in `AdminPanelProvider::authMiddleware` AFTER `RequireAdminTwoFactor` so enrollment is enforced first. Logic: if `$user->hasRole('admin')` AND `two_factor_confirmed_at !== null` AND `session('admin_2fa_passed') !== true` → redirect to `route('admin.two-factor-challenge')`. Defense-in-depth — catches cookie-replay / direct-URL-paste scenarios where someone reaches `/admin` without going through the custom login page.
- [ ] Rate limit: `RateLimiter::for('admin-two-factor-challenge', fn (Request $r) => Limit::perMinute(5)->by($r->session()->id()))` registered in `AppServiceProvider` + applied to the `POST` route via `throttle:admin-two-factor-challenge`. Five wrong codes per minute per session.
- [ ] Logout clears `admin_2fa_passed` automatically — Filament's `AuthenticateSession` middleware (already in panel middleware) regenerates the session on logout, killing the marker. No extra wiring needed.
- [ ] React page `resources/js/pages/admin/two-factor-challenge.tsx` — Stakly-skinned (dark palette, `border-glow` card, gradient submit button). Single OTP-style 6-digit input with auto-advance, "Use recovery code instead" toggle that swaps to a single text input. Activate the `ui-ux-pro-max` skill before building. Uses `useForm` from `@inertiajs/react`. Layout: full-screen centered (no `SiteLayout` chrome — this is an auth bottleneck surface).
- [ ] Pest tests (in `tests/Feature/Admin/AdminTwoFactorChallengeTest.php`): (1) admin with 2FA logs in via Filament form → redirected to challenge page; (2) admin without 2FA → no challenge, lands directly on /admin (P3 enrollment gate catches the no-2FA case separately); (3) non-admin role → no challenge wiring fires; (4) valid TOTP code → lands on /admin + session marker set; (5) invalid code → stays on challenge with errors; (6) recovery code path: valid code accepted + marked used in DB; (7) re-visit /admin in same session after passing → no re-challenge; (8) logout + re-login → fresh challenge fires; (9) rate limit: 6th attempt in a minute returns 429; (10) middleware defense: directly hit /admin without going through login page → bounced to challenge.

Open question for Phase 6 (decide at build time): does the "Cancel / I lost my TOTP device" escape hatch log the admin out immediately and surface a flash message instructing them to email support, or does it route to a dedicated recovery-codes-only page first? Recommend logout + flash — recovery codes already act as the in-band escape; the cancel button is the out-of-band one. Revisit when wiring.
