...
- **M26 Phase 4** — Full-site i18n. Slices A/B/C ✓ 2026-06-04 + global `URL::defaults` fallback ✓ 2026-06-05. **Slices D-1 through D-10 (Listings + Profile + Match + Wallet + Notifications + Settings) ✓ 2026-06-05** — listings (D-1..D-5), public profile (D-6), match (D-7a..D-7e), wallet (D-8), notifications (D-9), settings layout + profile + security + 2FA + linked-accounts + backend (D-10). **Up next: D-11 Auth → D-12 BannedBanner / M30 user-facing → D-13 Validation/flash/errors**. Translation labor (`lang/ka.json` / `lang/ru.json` content) tracked separately as a content backlog.
