...
- **M43 P4 — team-play lobby design polish** (`/design-review` on `/listings/262`, 2026-07-05) — **in progress.** Four fixes to the 5v5/2v2 lobby view: (1) **join action** — the open-slot join was faint dashed text and guests hit a dead-end "Open slot"; now it's a clearer CTA (solid tint + `UserPlus` icon) and **guests get a "Sign in to join"** affordance (opens the auth modal in place); (2) **countdown demote — tried then REVERTED** per user (the "Lobby fills within" clock is back at its original `text-2xl sm:text-3xl`); (3) **Team A/B identity** — added a subtle team accent (Team A pink / Team B purple avatar rings on header + slot cards, a team-colored fill-count dot) so the two sides read as distinct; (4) **softened empty states** — a player with zero settled matches now shows a quiet placeholder column instead of five repeated faded "N" cells (partial form still pads with N for context). Touches `lobby-header`, `team-slot-column`, `slot-card`, `recent-form-strip`. Verified in-browser (guest + authed, 375/1280).
