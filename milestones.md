...
- **M34 P5 — Private lobby access hardening** — *in progress 2026-06-28*. Closes the documented "URL = access" gap: `ListingPolicy::viewLobby` / `joinLobby` + `LobbyChannel::join` only checked `isTeamPlay()`, and the invite token was a no-op redirect — so enumerating sequential `/listings/{id}` exposed **and** let anyone join a *private* lobby (roster + stakes). **Decisions (2026-06-28, with the user):**
...
- [x] **Lobby `SlotCard` → FACEIT-roster layout.** Header (avatar + name left | ELO + level dial + Ready right), divider, an **"OVERALL" stat block** (Matches / Win rate / Completion-30d), and a **flush full-height vertical W/L column** on the right edge — always **5 slots**, padded with a faded **"N"** for no-match-yet. Full height via `self-stretch` (NOT `h-full`, which collapsed it to content size). `EmptySlot` height bumped to `128px` to track. Dial is `dialSize={32}` here.
- [x] **FACEIT level dial** (`faceit-rating-badge.tsx`) reworked to the reference look: bare **`ELO (dial)`** — number first, dial right, **no pill, no "ELO" label**. Configurable **`dialSize`** prop (marketplace default **22**, lobby **32**) + tunable module consts **`RING_THICKNESS` (0.11)** / **`LEVEL_NUMBER_SIZE` (0.4)**. Colors corrected to the **official FACEIT ladder** (grey L1 / green L2–3 / yellow L4–7 / orange L8–9 / red L10) — 5 `--faceit-*` CSS vars in `app.css`.
...
- [ ] **Responsive verification** at 375 / 768 / 1024 — the bare-rating swaps don't change layout (both badges `shrink-0` inline; `md:` breakpoints untouched), but a visual pass is still owed.
