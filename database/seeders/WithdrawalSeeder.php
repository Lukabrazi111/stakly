...
class WithdrawalSeeder extends Seeder
{
    ...
    private function seedWithdrawals(User $user): void
    {
        ...
        Withdrawals::send($completed);

        $rejected = Withdrawals::request($user->fresh(), '75', self::TRC20_SAMPLE);
        Withdrawals::reject($rejected, 'Destination address failed our screening check.', $admin ?? $user);

        $failed = Withdrawals::request($user->fresh(), '40', self::TRC20_SAMPLE);
        Withdrawals::markFailed($failed, 'Provider reported insufficient hot-wallet liquidity.');

        // Left Pending: its ProcessWithdrawal job is faked, so it stays queued.
        Withdrawals::request($user->fresh(), '60', self::TRC20_SAMPLE);
    }
