...
### Post-launch hardening (2026-06-28 → 2026-06-30)

Two follow-on phases + a UI revert, after the June 12–14 core shipped (these reuse the P5/P6 labels of the original phases above — distinguished here by date):

- **Private lobby access hardening (2026-06-28; suite 1675 green).** Closed the "URL = access" gap — `viewLobby`/`joinLobby` + `LobbyChannel::join` only checked `isTeamPlay()` and the invite token was a no-op redirect, so enumerating `/listings/{id}` exposed *and* let anyone join a private lobby (roster + stakes). New `App\Support\LobbyInvitePass` (per-session pass, single session-key source); `/lobbies/{token}` grants the pass, then the canonical URL authorizes only creator / live participants / pass-holders — everyone else **404s** (hides existence, never 403; the redirect drops 301 → 302 so the pass route always runs). Login required to view a private lobby; a kicked player (`kicked_at` row; voluntary leave *deletes* the row, so leavers keep access) is barred re-entry. Gate centralised in `ListingPolicy`; `LobbyChannel` delegates via `Gate::allows('viewLobby')` so page + channel auth can't drift. Decisions (with user): token=key, login-to-view, rotation deferred, kick-bars-re-entry. Pest: guessed-id→404, token→access, join-blocked-without-pass, kicked→barred, leaver→fine, public→open, guest→login.
- **Invite-link share popover (2026-06-30).** Replaced the always-visible `LobbyInviteBanner` (deleted) with an on-demand **Share popover** off the lobby-header Share button (mirrors `ShareProfileButton`: heading + QR + copyable link + Copy + toast). Fixed a real bug — the old Share copied `window.location.href` (the `/listings/{id}` page), which 404s for any invitee of a private lobby after the access hardening above. The popover shares the link that actually grants access: invite-token URL (`/lobbies/{token}`) for a private owner, public listing URL for public lobbies, **hidden** for a private non-owner. Owner-gated by the payload (`invite_token` is owner-only in `LobbyResource`); lifecycle-gated to recruiting/ready_checking. Trigger labelled "Invite" (private owner) / "Share" (public).
- **Slot-card polish (2026-06-29) — mostly reverted.** Kept: removed the magenta `border-glow` on the invite banner. **Still open (tracked in `milestones.md`):** the slot-card kick-button placement — the top-right ✕ overlaps the FACEIT dial; several redesigns were tried and reverted, needs fresh direction.
...
## M42 — Filament admin audit fixes (multi-game + team-play + perf) ✅ shipped 2026-06-30

Triggered by a user-reported `UnhandledMatchError` 500 on `/admin/listings`. A 55-agent adversarial audit of the whole admin panel surfaced **48 confirmed issues** the admin never caught up with as M15 (FACEIT/Steam providers, Cs2/Dota2 games), M34 (team lobbies) and M41 (ratings) landed — 4 crashes, 16 bugs, 15 ux, 7 perf — all fixed (money-adjacent dispute/listing/wallet code hands-on; widgets, CMS, games, user-infolist, chat-roles via a 9-agent fix workflow). **Crashes:** non-exhaustive `match()` over `LinkedAccountProvider` (`ListingsTable`/`ListingInfolist`) + over `AutoFetchOutcome::AcIncomplete` twice in `GameMatchInfolist` (500'd the dispute page for any FACEIT AC-incomplete result) → fixed by making the enums own their display (`displayName()` / `label()` / `badgeColors()`). **Team-play blindspots:** the dispute view was 1v1-shaped (creator + blank taker, pot hardcoded `stake×2`, provider handles read LIVE + chess-only) → rebuilt with a **Team A / Team B roster** (live `lobby_participants` for identity + at-lock `match_provider_snapshots` for the verified FACEIT/Steam handle + rating), team-aware money (pot = stake × team_size × 2 + per-player share), snapshot-sourced handles for all 4 providers, eager-loaded on `ViewGameMatch::resolveRecord`. Admin `ViewListing` force-cancel ran the 1v1 `CancelListingAction` on team lobbies (mis-refunds pooled escrow) → gated 1v1 to `! isTeamPlay()` + new `App\Actions\Lobby\Admin\ForceCancelTeamLobbyAction`. `UserInfolist` open-disputes used `forParticipant` (team roster members invisible) → `forRosterParticipant`. Wallet money-trail couldn't follow team settlements (suffixed per-player `reference_id`s) → siblings group by `related_listing_id`. **Multi-game/provider + perf:** `ProviderCircuitBanner` now tracks FACEIT; widget per-poll COUNT-loops collapsed to single GROUP-BY range queries; chat-roles team-aware (Team A/B). **Games "Active" gate** tightened to require `Game::hasArbitrationDriver()` (chess + cs2 ✓, dota2 ✗) — which exposed the gate had been a silent no-op (array `->rules([fn (Get $get)…])` is handed raw to Laravel; the enum-options `$get('status')` returns the enum) → `->rule()` + enum-or-string normalize. Backend **1691** green; new `M42MultiGameAdminTest` / `GameResourceGateTest` / `ForceCancelTeamLobbyTest` + extended parser/resource tests. **Adjacent player-side fix (same session):** the match-page "Money breakdown" pill (`match-details-strip.tsx`) was a hover-only Radix HoverCard → never opened on mobile tap + preventDefault'd `touchstart` in React's passive listener (console spam) → made device-aware via a new `useHasHover()` hook (HoverCard on hover devices, Popover on touch).

### Decisions

- **Enums own their display** (`displayName()` / `label()` / `badgeColors()`) so admin code never inline-`match()`es a growable enum — a new case becomes a compile-time obligation, not a runtime 500 on the dispute page.
- **Dispute view is team-aware:** roster from `lobby_participants` (Stakly identity + profile link) annotated with `match_provider_snapshots` (the immutable at-lock verified handle + rating — survives mid-match unlinks, covers all providers); money = `stake × team_size × 2`.
- **Force-cancel splits by shape:** 1v1 → `CancelListingAction` (gated `! isTeamPlay()`); team → `ForceCancelTeamLobbyAction` (refund-all-ready + vacate + cancel, idempotent per-player refs). The 1v1 action mis-refunds a team's pooled escrow.
- **Wallet trail groups by `related_listing_id`** (every escrow + settlement row carries it; one listing = one match's full hold/payout/fee/refund set) — captures suffixed team per-player refs the prefix parser can't.
- **Games "Active" requires an arbitration driver**, not bare enum membership (tightens the M24 rule) so an admin can't advertise adapter-less Dota2 as live.
- **Gotchas worth keeping:** (1) never put a static idempotency `reference` on a *repeatable* toggle (`ToggleReadyAction` Ready/un-Ready) — Wallet idempotency returns the prior hold on a re-fire and holds nothing, shorting the pot (a fix-agent did this; caught + reverted). (2) Filament cross-field validation needs `->rule()` (singular, injected), not `->rules([])` (handed raw to Laravel), and `$get()` on an enum-options Select returns the enum instance, not its value. (3) Radix HoverCard is hover-only + preventDefaults `touchstart` in React's passive root listener — for anything mobile users tap, branch to Popover via `useHasHover()`.

---

## M43 — Listings & lobby design polish ✅ shipped 2026-07-04 → 2026-07-05

Four `/design-review`-driven passes tightening the listings + lobby visual system. All verified in-browser at 375/1280 (tsc / prettier / console clean); no backend changes.

- **P1 — CS2 listings design polish (2026-07-04, PR #28).** Cut gradient overload: per-card CTA → new `outlinePrimary` button variant (transparent, fills solid pink on hover) so the gradient stake anchors each card; avatar fallbacks → `bg-primary/15` tint (grid / row / create-preview / detail). Game-aware translated tab title (was hardcoded "Browse chess listings"); game-tab pills → 44px, view toggle → 40px; card names promoted to `<h2>`. Kept the single glow CTA on the detail booking widget (one hero action). Audit scored **B / AI-slop A−**.
- **P2 — merge + recolor game/platform chips (2026-07-04, PR #29).** The loud orange FACEIT pill + separate game chip merged into one `GameChip` (`CS2 · 2v2 · FACEIT`, optional `platform` prop). Platform text tinted via new `--platform-*` tokens (FACEIT orange, chess.com gold, Lichess neutral, Steam blue) — a calm identity colour, not a filled pill; format (NvN) stays accent purple. The generic lucide game icon removed (text-only chip, every `GameChip` surface).
- **P3 — listing-detail chip + match-details polish (2026-07-05, commit `27d8582`).** Decluttered the identity chip strip: dropped the duplicate bare "played-on" platform chip (moved into Match details as a `Platform` row); filtered linked-account chips to the listing's game (chess → chess.com/Lichess only, drops the FACEIT/CS2 account noise), recoloured calm with the tokens; folded Joined into the trust line. Match details rebuilt from a 2-col icon grid into a label→value **spec-sheet** (hairline dividers) rhyming with the booking-widget stake breakdown. Platform label/colour/href + a new game→providers relevance map centralised into `resources/js/config/platforms.ts` (was duplicated across `game-chip` / `verification-chip` / `verified-platform-chip` / `show.tsx`).
- **P4 — team-play lobby polish (2026-07-05).** Open-slot join → clearer CTA (solid tint + `UserPlus`) and guests get "Sign in to join" (opens the auth modal in place) instead of a dead-end "Open slot"; Team A pink / Team B purple avatar rings + fill-count dot so the two sides read as distinct; empty W/L form column softened to a single "—" (partial form still pads with "N"). A countdown-demote was tried then reverted per user. Touches `lobby-header` / `team-slot-column` / `slot-card` / `recent-form-strip`. **Security note (raised same session):** a cancelled *public* lobby is viewable from incognito **by design** (public = URL-access; the `viewLobby` gate never checks status); *private* lobbies still 404 guests (M34 access gate; `PrivateLobbyAccessTest` 11/11 green).
- **P5 — profile shows all verified linked accounts (2026-07-05).** The profile header (`profile-header.tsx`) hardcoded only chess.com + Lichess chips (built pre-M15), so a user's verified **FACEIT / Steam** accounts never rendered on their own profile even though they show on listing detail (found via the testuser profile — FACEIT was verified in the DB but invisible). `UserProfileResource` now exposes a full verified `linked_accounts` array (`[{provider, username}]`, mirrors `ListingResource`, filtered to `verified_at`); the header iterates it via the shared `VerificationChip`. Profile is game-agnostic identity → shows ALL verified accounts, no game filter (unlike the listing detail's relevance filter). Legacy `chess_com_username`/`lichess_username` fields kept for other consumers. Follow-on: with FACEIT now filling the chip row, the **"Joined" date moved out of the account-chip strip up to the `@handle` line** ("@user · Joined Jul 2026") — it was reading as a fourth account chip; the chip row is now purely clickable verified accounts (and hides entirely when the user has none). Also reconciled the **settings edit-preview** (`profile-preview.tsx`) to the same layout, and fixed the **stale `auth.ts` `linked_accounts.provider` type** (was `'chess_com' | 'lichess'` pre-M15 → `ListingPlatform`; FACEIT/Steam already rendered at runtime, the type just under-described it).
...
## M46 — Harden chess result auto-detection (verified match results) ✅ shipped 2026-07-07

Made chess result auto-detection reliably land the right game on its own, keeping the after-the-fact API watch-and-search model (no game orchestration, no URL-paste — both parked). Planning via `/office-hours` + `/plan-eng-review`; a gstack `/review` pass then caught a real security bug the structural reviews missed. Branch `feat/m46-chess-disambiguation`.

- **P1 — Failure-mode audit (post-launch analysis, no dev build).** Inherently a data task: once there's real traffic, query `match_auto_fetch_attempts` (ad-hoc SQL — group by outcome + `outcome_reason` per provider, keying off the last non-skipped attempt so `not_pending` page-visit rows don't skew it) to see the true failure-mode distribution, then tune the P3 retry windows / P4 guidance to what the data shows. Nothing to build in dev (data is seeded/meaningless); the admin "Why it's in review" panel (P5) already surfaces the reason per-match. A one-off `stakly:auto-fetch-audit` reporting command was built + Pest-validated during closeout, then removed as unnecessary — the query is trivial ad-hoc when the moment comes.
- **P2 — Disambiguation hardening (SECURITY).** `rejectStale()` in both auto-fetch jobs drops any game whose *start* pre-dates the stake (closes the single-candidate direct-settle bypass to pre-play reuse) + deterministic "first game started after the stake" pick + new `stale_game_rejected` audit reason. TC kept as a multi-candidate disambiguator only (M33 revert respected).
- **P3 — Window + retry + stream hardening (latency-only).** Confirmed the Lichess real-time stream is already fully wired (`LichessStreamCommand` sidecar). chess.com no-match retry tail `[5,15,45]`→`[5,15,45,90,150]` (~5 min, `uniqueFor` 360→600); NEW Lichess chain `[3,8,20]` (~31s) to bridge export-index lag; NEW young-match cron tier `stakly:auto-fetch-pending --min-age-minutes=1 --max-age-minutes=10` @everyMinute closing the [chain-exhaust→10-min-cron] seam → chess.com settlement ~10-15 min → ~1-2 min. 5-lens adversarial review passed. Accepted trade-off: the provider-agnostic young cron amplifies ~4 calls/min per stuck Lichess match during a stream OUTAGE (saturates the shared 60/min bucket at ~15 concurrent → latency, not a breaker freeze).
- **P4 — Player guidance UX (pure frontend).** Folded the precise instruction into the existing `WaitingForGameCard`: chess 1v1 now reads "Play your rated {Blitz} game on {chess.com}" + "Start it now and play a single game to a finish — Stakly reads the result automatically…" (all four finder constraints: rated · exact TC · one game · started now). Branches on time-control presence so CS2/team keeps the generic copy (M47 owns CS2 guidance).
- **P5 — Residual ambiguity → cleaner ManualReview (display-only, zero money code).** Admin: `MatchAutoFetchAttempt::reasonLabel()` map turns raw `outcome_reason` codes into plain English + a "Why it's in review" infolist section synthesising the terminal (last non-skipped) attempt. Player: `admin-review-banner` branches `manual_review` copy on `dispute.opened_by_id` (accurate timeout-vs-dispute) + "what happens next / escrow / evidence". Granular detection reasons stay ADMIN-ONLY (coarse-safe player copy so the pre-play guard isn't leaked to cheaters).
- **`/review` pass (gstack — 4 specialists + adversarial).** **[real fix] The P2 guard was a NO-OP on chess.com** — verified against the LIVE chess.com API (78 real blitz games): live games carry **no `start_time`** (daily-only), so `parseGame`'s `start_time ?? end_time` fell back to `end_time` and `rejectStale` became redundant with the archive search's own `end_time>=since` filter; **hidden because the test fixture fabricated a `start_time` that blitz games never return.** Fixed via `ChessComGameClient::parseStartTimestamp()` — parses the true start from the PGN `[UTCDate]`+`[StartTime]` (UTC), end_time floor only if unparseable; fixture rewritten to the real shape; + client/composition regression tests. Also fixed: N+1 in the cron loop (`->with(['listing','providerSnapshots'])`), page-visit write amplification (`GameMatchController::show` gates the auto-fetch dispatch on `Pending`), and a copy-branch tweak. Deferred (user call): extract the now-identical retry-tail helpers into a shared trait when FACEIT adopts the no_match chain (M47).

**Decisions.**
- **chess.com live games have NO `start_time`** (that field is daily-chess only); the true start is in the PGN `[UTCDate]`+`[StartTime]` (UTC). Any future chess.com timing work must read the PGN, never assume `start_time`.
- Watch-and-search kept over orchestration / URL-capture — lowest-friction, provider-agnostic (covers chess.com + Lichess), no new OAuth scopes / paste step; both parked, revisit only if hardening proves insufficient.
- Collusion (two players agreeing a result off-platform) = accepted risk — the API faithfully reports the agreed outcome, so detection can't stop it.
- P1 is a post-launch analysis step (ad-hoc query on `match_auto_fetch_attempts`), not a dev build — the distribution only means something on real traffic.
