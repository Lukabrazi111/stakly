...
**Slice 3d — Single-candidate time-control enforcement.** Slice 3c's picker is now called for ALL candidate counts (no longer "multi-only"). A blitz-listing matched on a bullet game no longer auto-settles — outcome=ambiguous + outcome_reason='time_control_mismatch', match stays Pending until either a matching game lands or the M16 timeout flips it to ManualReview. Test helpers in 4 files updated to default the listing's `time_control` to `['blitz', 'rapid', 'classical']` (catch-all) so happy-path tests pass deterministically; tests that exercise TC mismatch override explicitly.
