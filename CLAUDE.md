...
- **Choose tech on merit, not on speed-to-ship.** When two implementations are available, evaluate them on capability, correctness, UX, and performance — never on "this is faster for me to type." If the better-fit infrastructure already exists in the project, default to it; the cost of wiring it up is paid once, the cost of the inferior approach compounds forever. Concrete cases this catches: **broadcast (Reverb) over polling** for any live state (lobby roster, chat, notifications, match state) — Reverb is already wired for chat, extending it is the right call, polling is a code smell; **DB-backed seeded data** over hardcoded route-closure props; **Action classes** over inline controller business logic; **Wallet service** over any direct balance write; **Form Requests** over inline `$request->validate(...)` for non-trivial input; **framework primitives** (Laravel's cache tags, queue middleware, policy gates) over hand-rolled equivalents. If a borderline trade-off makes the shortcut the right call (e.g. genuinely one-shot, throwaway, scope-locked), surface it explicitly so it's a deliberate decision, not a default. The trigger phrase to watch for in your own reasoning: "ship it simple" / "6 lines vs 60" / "we can revisit later" — those are usually the wrong framing.
