...
- **Chain v1**: TRC20 (Tron USDT). **Chain v2**: BEP20 (BSC USDT) committed for post-launch. Integration uses Tatum's Custodial Managed Wallets API.
...
- **Chain integration uses Tatum's Custodial Managed Wallets API behind the `ChainGateway` adapter.** All blockchain calls flow through `App\Services\Chain\ChainGateway`; never let Tatum SDK calls or raw HTTP requests leak into controllers, the `Wallet` service, or anywhere else. Tatum holds per-user TRC20 keys server-side; we orchestrate via REST + receive deposit notifications via webhooks. Internal ledger stays in `wallet_transactions` (Tatum's Virtual Accounts feature is unused). Key-export is the migration escape hatch if we ever need to swap providers. Full architecture in `milestones.md` M9 + pre-launch gate.
