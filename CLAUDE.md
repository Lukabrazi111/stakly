...
- **Chain integration (M9) — NowPayments selected as the payment gateway; not wired yet.** The provider-agnostic half is done (M9 P0a + P0b): the ledger, payout clearing, account freeze, and the full withdrawal lifecycle all work end-to-end against `MockGateway`, and `WalletController::withdrawStore` writes for real. What's missing is the provider edge — `users.tron_address` still comes from `App\Support\MockTronAddress` (placeholder, not on-chain), and **there is no deposit webhook receiver, so deposits cannot be credited at all; balances come from seeders.** Deposit attribution will use permanent per-user addresses (not per-payment invoices, and not NowPayments Custody sub-accounts — those would put per-user balances on the provider's side and drift from our ledger). Remaining work is tracked in `docs/billing-roadmap.md`. No NowPayments client / webhook receiver / API-key wiring without explicit go-ahead. (Provider is custodial — they hold keys; our side will be API client + webhook receiver only. No Solidity / on-chain signing / key-storage code.) The internal ledger (`wallet_transactions` + `App\Services\Wallet`) is provider-agnostic and stays as source of truth.
