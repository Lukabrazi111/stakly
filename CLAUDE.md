...
- **Chain integration (M9) is paused pending a specialist.** Until then: `users.tron_address` is populated by `App\Support\MockTronAddress` (placeholder, not on-chain), the deposit page shows the mock address, and `WalletController::withdrawStore` short-circuits with a notice toast (no ledger write). No chain SDKs / signing libraries / webhook endpoints / key-storage code without go-ahead. The internal ledger (`wallet_transactions` + `App\Services\Wallet`) is provider-agnostic and stays as source of truth.
